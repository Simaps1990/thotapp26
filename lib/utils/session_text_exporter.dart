...
abstract final class SessionTextExporter {
  static String buildSummary({
    ...
    buffer.writeln(
      '📅 ${_capitalizeFirstLetter(dateFormat.format(session.date))}',
    );
    if (session.location.trim().isNotEmpty)
      buffer.writeln('📍 ${session.location}');
    if (session.shootingDistance != null &&
    ...
    if (session.weatherEnabled) {
      ...
      if (hasAnyWeatherField) {
        ...
        if (session.temperatureEnabled && session.temperature.isNotEmpty) {
          buffer.writeln(
            strings.textExportTemperature(
              converter.parseTemperatureString(session.temperature),
            ),
          );
        }
        if (session.windEnabled && session.wind.isNotEmpty) {
          buffer.writeln(
            strings.textExportWind(
              converter.parseWindSpeedString(session.wind),
            ),
          );
        }
        if (session.humidityEnabled && session.humidity.isNotEmpty)
          buffer.writeln(strings.textExportHumidity(session.humidity));
        if (session.pressureEnabled && session.pressure.isNotEmpty)
          buffer.writeln(strings.textExportPressure(session.pressure));
        buffer.writeln();
      }
    }
    ...
    buffer.writeln(
      strings.textExportAvgPrecision(
        session.hasCountedPrecision
            ? '${session.averagePrecision.toStringAsFixed(1)}%'
            : '—',
      ),
    );
    ...
    if (session.exercises.isNotEmpty) {
      ...
      for (int i = 0; i < session.exercises.length; i++) {
        ...
        final equipmentNames = _resolveEquipmentNames(
          ex.equipmentIds,
          provider,
        );
        ...
        buffer.writeln();
        buffer.writeln(
          '${strings.textExportExerciseN(i + 1)}${ex.name.trim().isEmpty ? '' : ' — ${ex.name}'}',
        );
        buffer.writeln(
          '   ├─ ${strings.textExportPlatformLabel}: $platformName',
        );
        buffer.writeln('   ├─ ${strings.textExportAmmoLabel}: $ammoName');
        if (equipmentNames.isNotEmpty)
          buffer.writeln(
            '   ├─ ${strings.textExportEquipmentLabel}: ${equipmentNames.join(', ')}',
          );
        if (ex.targetName != null && ex.targetName!.trim().isNotEmpty)
          buffer.writeln(
            '   ├─ ${strings.textExportTargetLabel}: ${ex.targetName}',
          );

        final hasSteps = ex.steps != null && ex.steps!.isNotEmpty;
        if (!hasSteps) {
          buffer.writeln(
            '   ├─ ${strings.textExportDistanceLabel}: ${converter.formatDistance(ex.distance)}',
          );
          buffer.writeln(
            '   ├─ ${strings.textExportShotsFiredLabel}: ${ex.shotsFired}',
          );
          if (ex.isPrecisionCounted)
            buffer.writeln(
              '   ├─ ${strings.textExportPrecisionLabel}: ${ex.precision!.toStringAsFixed(1)}%',
            );
          if (ex.observations.trim().isNotEmpty) {
            buffer.writeln(
              '   └─ ${strings.textExportNotesLabel}: ${ex.observations}',
            );
          } else {
        ...
        for (int si = 0; si < ex.steps!.length; si++) {
          ...
          if (st.shots != null)
            details.add(strings.textExportStepShots(st.shots!));
          if (st.distanceM != null) details.add('${st.distanceM} m');
          if ((st.target ?? '').trim().isNotEmpty)
            details.add(strings.textExportStepTarget(st.target!.trim()));
          if (st.reloadType != null)
            details.add(strings.textExportStepReload(st.reloadType!.name));
          if (st.durationSeconds != null)
            details.add('${st.durationSeconds} s');
          if ((st.trigger ?? '').trim().isNotEmpty)
            details.add(strings.textExportStepTrigger(st.trigger!.trim()));
          if ((st.platformFrom ?? '').trim().isNotEmpty ||
              (st.platformTo ?? '').trim().isNotEmpty) {
            details.add(
              strings.textExportStepTransition(st.platformFrom, st.platformTo),
            );
          }

          buffer.writeln(
            '   │   $icon $idx — ${st.type.name}${details.isEmpty ? '' : ' · ${details.join(' · ')}'}',
          );
        ...
        buffer.writeln(
          '   ├─ ${strings.textExportAutoTotal(ex.detailedTotalShots)}',
        );
        buffer.writeln(
          '   ├─ ${strings.textExportAutoMaxDistance(ex.detailedMaxDistance ?? 0)}',
        );
        if (ex.isPrecisionCounted)
          buffer.writeln(
            '   ├─ ${strings.textExportPrecisionLabel}: ${ex.precision!.toStringAsFixed(1)}%',
  ...
  static List<String> _resolveEquipmentNames(
    List<String> ids,
    ...
    for (final id in ids) {
      ...
      final accessory = provider.accessories
          .where((a) => a.id == id)
          .firstOrNull;
      if (accessory != null && accessory.name.trim().isNotEmpty)
