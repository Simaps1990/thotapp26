...
Diagnostic _diagnostic({String id = 'd1', String platformId = 'w1'}) =>
    Diagnostic(
      incidentKey: 'none',
...
void main() {
  ...
  group(
    'ThotProvider — verrouillage plan gratuit (helpers UI)',
    skip: 'Freemium limits disabled via _kFreeLimitsDisabled flag',
    () {
      test(
        'isPlatformLockedForFree : index 0 libre, index 1+ verrouillé',
        () async {
      ...
      test(
        'isAmmoLockedForFree : index 0 libre, index 1+ verrouillé',
        () async {
          final p = await _makeProvider();
          final a = _ammo();
      ...
      test(
        'isSessionLockedForFree : toujours false (sessions illimitées)',
        () async {
          final p = await _makeProvider();
          final s = _session();
      ...
      test(
        'isItemDocumentLockedForFree : index 0 libre, index 1+ verrouillé',
        () async {
          final p = await _makeProvider();
  ...
  group('ThotProvider — recordPlatformPartChange', () {
    test('ajoute une entrée historique de type piece', () async {
      ...
      expect(w.history.first.type, PlatformHistoryType.partReplacement);
      expect(w.history.first.data[PlatformHistoryDataKey.partName], 'Percuteur');
    });
